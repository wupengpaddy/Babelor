...
def json2xml(js: str):
    return ElementTree.tostring(dict2etree(json2dict(js)), encoding=ENCODING).decode(ENCODING)


def dict2xml(dt: dict):
    return ElementTree.tostring(dict2etree(dt), encoding=ENCODING).decode(ENCODING)
