...
def worker(select_date: str):
    """
    :param select_date: str    提取数据日期
    :return: None
    """
    # 数据范围
    ...
    sql = SQL(conn)
    afds_file_path = SqlData(AFDS_SQL.format(start_date.strftime(DateFormat),
                                             end_date.strftime(DateFormat)), "afds").save()
    cdm_file_path = SqlData(CDM_SQL.format(start_date.strftime(DateFormat),
                                           end_date.strftime(DateFormat)), "cdm").save()

    # 读取 Excel 数据源
    afds = pd.read_excel(afds_file_path, dtype=str, index_col=0)
    cdm = pd.read_excel(cdm_file_path, dtype=str, index_col=0)
    ...
    # email_send(select_date.replace("-", ""), pvg)
    # ftp_send(select_date.replace("-", ""), pvg)
    # json_send(select_date.replace("-", ""), pvg)
    pvg.to_excel("data/{0}.xlsx".format(select_date.replace("-", "")))
...
def range_worker(year: int):
    ...
    for i in range((end_date - begin_date).days + 1):
        ...
        worker(select_date.strftime(DateFormat))
